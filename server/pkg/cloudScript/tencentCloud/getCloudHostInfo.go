...
func (s *TencentCloudService) GetCloudInsInfo(region string, publicIpv4 string, publicIpv6 string, insName string, cloudPid string, offset int64, limit int64) (*HostResponse, error) {
	...
	if publicIpv4 != "" {
		...
		request.Filters = append(request.Filters, &cvm.Filter{
			...
			Values: common.StringPtrs([]string{insName}),
		})
	} else if cloudPid != "" {
		request.Filters = append(request.Filters, &cvm.Filter{
			Name:   common.StringPtr("project-id"),
			Values: common.StringPtrs([]string{cloudPid}),
		})
	}
