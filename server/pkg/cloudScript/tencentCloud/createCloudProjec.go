...
func (s *TencentCloudService) CreateCloudProject(projectName string) (err error) {
	...
	// 返回的resp是一个AddProjectResponse的实例，与请求对象对应
	response, err := client.AddProject(request)
	...
	// 输出json格式的字符串回包
	logger.Log().Info("Project", "CreateProjectGroup", response)
	return nil
}
