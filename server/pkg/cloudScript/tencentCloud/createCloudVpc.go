...
func (s *TencentCloudService) CreateCloudVpc(region string, vpcName string, cidrBlock string) error {
	...
	request.VpcName = common.StringPtr(vpcName)
	request.CidrBlock = common.StringPtr(cidrBlock)

	// 返回的resp是一个CreateVpcResponse的实例，与请求对象对应
	_, err := client.CreateVpc(request)
