...
func (s *TencentCloudService) CreateCloudVpcSubnet(region string, vpcId string, subnetName string, subnetCidrBlock string, zone string) error {
	...
	request.CidrBlock = common.StringPtr(subnetCidrBlock)
	request.Zone = common.StringPtr(zone)

	// 返回的resp是一个CreateSubnetResponse的实例，与请求对象对应
	_, err := client.CreateSubnet(request)
